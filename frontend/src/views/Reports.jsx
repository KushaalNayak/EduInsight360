...
const PrintableReport = ({ students }) => (
    <div className="hidden print:block p-10 bg-white min-h-screen text-slate-900">
        <div className="text-center border-b-4 border-slate-900 pb-8 mb-10">
            <h1 className="text-4xl font-black uppercase tracking-tighter">EduInsight360 Analytics</h1>
            <p className="text-sm font-bold uppercase tracking-widest mt-2">Official Academic Performance Registry • {new Date().toLocaleDateString()}</p>
        </div>
        
        <table className="w-full border-collapse border border-slate-300">
            <thead>
                <tr className="bg-slate-100">
                    <th className="border border-slate-300 p-4 text-left font-black uppercase text-xs">Student ID</th>
                    <th className="border border-slate-300 p-4 text-left font-black uppercase text-xs">Full Name</th>
                    <th className="border border-slate-300 p-4 text-center font-black uppercase text-xs">CGPA</th>
                    <th className="border border-slate-300 p-4 text-center font-black uppercase text-xs">Attendance</th>
                    <th className="border border-slate-300 p-4 text-center font-black uppercase text-xs">Status</th>
                </tr>
            </thead>
            <tbody>
                {students.map((s, idx) => (
                    <tr key={idx}>
                        <td className="border border-slate-300 p-4 font-mono text-sm">{s.id}</td>
                        <td className="border border-slate-300 p-4 font-bold">{s.name}</td>
                        <td className="border border-slate-300 p-4 text-center font-black">{s.overallScore.toFixed(2)}</td>
                        <td className="border border-slate-300 p-4 text-center">{s.attendance}%</td>
                        <td className="border border-slate-300 p-4 text-center font-bold">{s.overallScore > 7 ? 'ELITE' : 'PASS'}</td>
                    </tr>
                ))}
            </tbody>
        </table>

        <div className="mt-20 border-t-2 border-slate-200 pt-10">
            <h3 className="text-lg font-black uppercase mb-4">Behavioural Assessment Summary</h3>
            <div className="grid grid-cols-2 gap-8">
                {students.map((s, idx) => (
                    <div key={idx} className="p-6 bg-slate-50 rounded-xl border border-slate-100">
                        <p className="font-black text-sm uppercase text-[#3366FF] mb-2">{s.name}</p>
                        <p className="text-xs text-slate-600 leading-relaxed font-bold">
                            Subject Proficiency: {s.overallScore > 9 ? 'Exceptional' : 'Standard'}. 
                            Attendance reflects {s.attendance > 90 ? 'strong' : 'moderate'} commitment to curriculum.
                            Recommended for advanced tracks based on current trajectory.
                        </p>
                    </div>
                ))}
            </div>
        ...
        <div className="mt-40 flex justify-between">
            <div className="text-center">
                ...
                <p className="text-[10px] font-black uppercase">Faculty Signature</p>
            </div>
            <div className="text-center">
                <div className="w-48 border-b border-slate-900 mb-2"></div>
...
const Reports = ({ role, showToast, students = [] }) => {
    const reportData = [
    ...
    const handlePrint = () => {
        showToast?.("Preparing printable summary...", 'info');
        setTimeout(() => window.print(), 1000);
    };

    const handleExportBatch = async () => {
        showToast?.("Generative CSV Archive...", 'info');
        await studentService.exportCSV();
        showToast?.("Batch Archive exported", 'success');
    };
    ...
    return (
        <>
            ...
            <PrintableReport students={students} />
        </>
    );
};
